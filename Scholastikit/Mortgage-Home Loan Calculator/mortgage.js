function init() {
  //Initialize calculator blank model
...
function setUIEvents() {
  ...
  const annualInsuranceErrMsg = document.getElementById(
    "annualInsurance-validity"
  );
  ...
  // Events for sliders
  sliderYearsMortgage.addEventListener("input", function (evt) {
    setInputFromSliderValue(evt.target, "inYears");
    chromeSliderStyling(evt.target);
  });

  sliderInterestRate.addEventListener("input", function (evt) {
  ...
  loanAmount.addEventListener("input", function (evt) {
    validateNumberInputText(
      evt,
      loanAmountErrMsg,
      evt.target.dataset.requiredErrorMsg
    );
  });

  annualTax.addEventListener("keypress", validateOnlyNumericKeys);
  annualTax.addEventListener("input", function (evt) {
    validateNumberInputText(
      evt,
      annualTaxErrMsg,
      evt.target.dataset.requiredErrorMsg
  ...
  annualInsurance.addEventListener("input", function (evt) {
    validateNumberInputText(
      evt,
      annualInsuranceErrMsg,
      evt.target.dataset.requiredErrorMsg
    );
  });

  // Focus box shadow when writing
  loanAmount.addEventListener("focus", function (evt) {
    setFocusBoxShadow(evt.target, 4);
  });

  loanAmount.addEventListener("blur", function (evt) {
    setFocusBoxShadow(evt.target, 0);
  });

  annualTax.addEventListener("focus", function (evt) {
    setFocusBoxShadow(evt.target, 4);
  });

  annualTax.addEventListener("blur", function (evt) {
    setFocusBoxShadow(evt.target, 0);
  });

  annualInsurance.addEventListener("focus", function (evt) {
    setFocusBoxShadow(evt.target, 4);
  });

  annualInsurance.addEventListener("blur", function (evt) {
    setFocusBoxShadow(evt.target, 0);
  });
...
function getErrorMessagesForScreenSize() {
  ...
  let loanAmountErrMsgStr =
    (annualTaxErrMsgStr =
    annualInsuranceErrMsgStr =
      "Mandatory field");
...
function setFocusBoxShadow(elem, shadowPx) {
  ...
  inputTextMask.style.boxShadow =
    "0 0 " + shadowPx + "px rgba(62, 61, 63, 0.62)";
}
...
function isFormValid() {
  const frmElems = document.querySelectorAll(
    "#form_mortgage_calc input.currency-text[required]"
  );
  let i;
  isNotValid = false;
...
function updateResultUIPanel(results) {
  ...
  const principleInterest = document.getElementById(
    "resultPrincipleAndInterest"
  );
  ...
  resultTotalInterest.innerHTML = results.totalInterest;
  resultTotalPayment.innerHTML = results.totalPayment;
...
function MortgageCalculatorModel(
  _interestRate,
  _loanAmount,
  _yearsOfMortgage,
  _annualTax,
...
MortgageCalculatorModel.prototype.computePrincipleInterest = function () {
  return (
    ...
    (1 - Math.pow(1 + this.interestRate / 100 / 12, -this.yearsOfMortgage * 12))
  );
};
...
MortgageCalculatorModel.prototype.computeMonthlyPayment = function () {
  return (
    this.computePrincipleInterest() +
    this.computeTax() +
    this.computeInsurance()
  );
};
...
MortgageCalculatorModel.prototype.getTotalInterest = function () {
  const monthlyPI = this.computePrincipleInterest();
  const totalPaid = monthlyPI * this.yearsOfMortgage * 12;
  return totalPaid - this.loanAmount;
};

MortgageCalculatorModel.prototype.getResults = function () {
  const principleInterestMonthly = this.computePrincipleInterest();
  const totalInterest = this.getTotalInterest();
  return {
    tax: this.computeTax().toFixed(2),
    annualInsurance: this.computeInsurance().toFixed(2),
    principleInterest: principleInterestMonthly.toFixed(2),
    monthlyPayment: this.computeMonthlyPayment().toFixed(2),
    totalInterest: totalInterest.toFixed(2),
    totalPayment: (parseFloat(this.loanAmount) + totalInterest).toFixed(2),
  };
};
