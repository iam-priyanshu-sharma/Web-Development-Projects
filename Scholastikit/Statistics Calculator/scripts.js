...
function calc() {
  ...
  output.value =
    ...
    range(sortedNums) +
    "\n\nFirst Quartile: " +
    firstQuartile(sortedNums) +
    "\n\nThird Quartile: " +
    thirdQuartile(sortedNums) +
    "\n\nInterquartile Range: " +
    (thirdQuartile(sortedNums) - firstQuartile(sortedNums)) +
    "\n\nMid-Range: " +
    midrange(sortedNums) +
    "\n\nMin: " +
...
function getInput() {
  ...
  let nums = document
    ...
    .value.split(document.getElementById("delim").value)
    .join("\n")
    .split("\n");
  for (let i = 0; i < nums.length; i++) {
    nums[i] = parseFloat(nums[i].trim()); //trim because whitespace doesn't get found in isNaN()
    if ((isNaN(nums[i]) || nums[i] == "") && nums[i] != 0) {
      //doesn't accept "" but does accept 0
      nums.splice(i, 1);
      i--;
    }
  }
...
function median(sortedArray) {
  if (!isValid(sortedArray)) return 0;
...
function thirdQuartile(sortedArray) {
  //TODO median of upper half (round to less items)
  if (!isValid(sortedArray)) return 0;
  let bottomHalf = sortedArray.slice(0, Math.floor(sortedArray.length / 2) - 1);
  return median(bottomHalf);
}

function firstQuartile(sortedArray) {
  //TODO median of lower half (round to less items)
  if (!isValid(sortedArray)) return 0;
  let topHalf = sortedArray.slice(
