...
function s(x, y) {
  ...
  var sum = 0;
  for (var i = 0; i < numArgs; i++) sum += s.arguments[i];
  return sum;
}
function p(x, y) {
  var numArgs = p.arguments.length;
  var sum = 0;
  for (var i = 0; i < numArgs; i++) sum += 1 / p.arguments[i];
  return 1 / sum;
}
//INTERFACE FUNCTIONS
...
function evaluateX(f) {
  if (checkInput(f.inFld.value)) f.outFld.value = eval(f.inFld.value).decn(N);
}
