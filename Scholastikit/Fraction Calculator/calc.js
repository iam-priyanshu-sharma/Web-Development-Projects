...
Fraction.prototype = {
  ...
  add: function (argument) {
    var num =
        ...
        argument.numerator * this.denominator,
...
function calc() {
  ...
  try {
    ...
    for (var i = 0; i < fractionsTemp.length; i++) {
      if (
        typeof fractionsTemp[i].numerator == "undefined" ||
        typeof fractionsTemp[i].denominator == "undefined" ||
        isNaN(fractionsTemp[i].numerator) ||
        isNaN(fractionsTemp[i].denominator)
      )
        return false;
    ...
    // * and /
    loop: for (var i = 0; i < operationsTemp.length; i++) {
